...
void RemoveWeaponConfig(uintptr_t Weapon, bool bspread)
{
	if (Weapon)
	{
		auto CurrentWeaponConfig = read<uintptr_t>(Weapon + 0x178);
		if (CurrentWeaponConfig)
		{
			auto FireSettings = read<uintptr_t>(CurrentWeaponConfig + 0x50);
			if (FireSettings)
			{
				auto SpreadModi = read<uintptr_t>(FireSettings + 0x98);
				if (!bspread)
					write<float>(FireSettings + 0x24, 0.0f), write<float>(FireSettings + 0x28, 0.0f);
				if (bspread && SpreadModi != NULL)
					write<float>(FireSettings + 0x2C, 0.0f), write<uintptr_t>(FireSettings + 0x98, NULL);
			}
		}
	}
}
...
auto CallHacks()->VOID
{
	while (true)
	{
		...
		if (CFG.b_NoRecoil)
		{
			RemoveWeaponConfig(GameVars.equipped_weapon_type, FALSE);
			write<uint64_t>(GameVars.equipped_weapon_type + 0x108, 0); // VerticalCurve
			write<uint64_t>(GameVars.equipped_weapon_type + 0x110, 0); // HorizontalCurve
			//Local + UTigerRangedWeaponComponent + UTigerRangedWeapon
				//+ VerticalRecoilCurve 0x0
				//+ HorizontalRecoilCurve 0x0

			//write<int>(0x4b8 + 0x4c8 + 0x248,50); 
			//GameVars.ranged_weapon_component = read<std::uint64_t>(GameVars.local_player_pawn + 0x4c8);
			//GameVars.ranged_weapon_component = read<std::uint64_t>(0x3a0 + 0x4c8 + 0x168);
			//write<int>(GameVars.equipped_weapon_type + 0x188, 50);
			//write<int>(GameVars.local_player_pawn + 0x4c8 + 0x168,50);
			//auto recoil = read<DWORD_PTR>(0x4b8 + 0x4c8 + 0x168);
			//write<float>(GameVars.equipped_weapon_type + 0x108, 0x0); // HorizontalCurve
			//write<float>(GameVars.equipped_weapon_type + 0x110, 0x0); // HorizontalCurve
			...
			//kernel::writemem(UTigerRangedWeapon + Offsets::Classes::UTigerRangedWeapon::VerticalRecoilCurve, &nullpointer, sizeof(nullpointer));
			//kernel::writemem(UTigerRangedWeapon + Offsets::Classes::UTigerRangedWeapon::HorizontalRecoilCurve, &nullpointer, sizeof(nullpointer));

			//uint64_t nullpointer = 0;

			//write<float>(0x168 + 0x108 & nullpointer, sizeof(nullpointer));
			//write<float>(0x168 + 0x110 & nullpointer, sizeof(nullpointer));
			Sleep(30);
		}
		if (CFG.b_Fastreload)
		{
			//auto RangedWeaponComponent = read<DWORD_PTR>(GameVars.local_player_pawn + 0X4c8);//UTigerRangedWeaponComponent
			//write<float>(GameVars.local_player_pawn + 0xb90 + 0x4c, 100.f);//ReloadSpeedMultiplier
			auto RangedWeaponComponent = read<DWORD_PTR>(GameVars.local_player_pawn + 0X4c8);//UTigerRangedWeaponComponent
		...
		{
			//auto RangedWeaponComponent = read<DWORD_PTR>(GameVars.local_player_pawn + 0X4c8);//UTigerRangedWeaponComponent
			//write<float>(RangedWeaponComponent + 0x198, 100.f);//CurrentFireRateMultiplier
			auto RangedWeaponComponent = read<DWORD_PTR>(GameVars.local_player_pawn + 0X4c8);//UTigerRangedWeaponComponent
			write<float>(RangedWeaponComponent + 0x198, 1.3f);//CurrentFireRateMultiplier
		}

		if (CFG.NoSpread) /*Ofc Add the Option */
		{
			RemoveWeaponConfig(GameVars.equipped_weapon_type, TRUE);
		}


		if (CFG.b_radar) /*Ofc Add the Option */
		{
		...
		if (CFG.b_eac1)
		{
			//	int Spin = 0; // add this att your actor loop start
			//
			//	if (0x3a0) continue;
			//	if (GetAsyncKeyState(VK_LBUTTON) < 0)
			//	{
			//		uintptr_t Mesh = read<uintptr_t>(GameVars.dwProcess_Base, GameVars.dwProcessId, 0x398 + 0x288);
			//		//write<FRotator>(Mesh + 0x128, FRotator{ 0, Spin, 0 });
			//		Spin + 20;
			//	}
				//	Testedand workin#
			system("taskkill /IM EasyAntiCheat_EOS.exe /F");
			system("taskkill /IM EasyAntiCheat_EOS.sys /F");

			//int retval = ::_tsystem(_T("taskkill /F /T /IM EasyAntiCheat.sys"));
			//int retval = ::_tsystem(_T("taskkill /F /T /IM EasyAntiCheat_EOS.exe"));

		}
		if (CFG.b_fastres)
		{
			read<DWORD_PTR>(GameVars.local_player_pawn);
			write<float>(GameVars.local_player_pawn + 0xcf0, 1337);
...
auto GameCache()->VOID
{
	while (true)
	{
		...
		//printf("Actor Count: %i", GameVars.actor_count);
		for (int index = 0; index < GameVars.actor_count; ++index)
		{
			...
			//if (name == ("TBP_Player_C"))
			{	
				//printf("Starting add of entity\n");
...
{
	//printf("Render Visual called.. \n");
	auto EntityList_Copy = entityList;

	for (int index = 0; index < EntityList_Copy.size(); ++index)
	{
		auto Entity = EntityList_Copy[index];
		//printf("Looping through entity list \n");
...
void Render()
{
	...
	{
		InputHandler();
		ImGui::SetNextWindowSize(ImVec2(500.000f, 600.000f), ImGuiCond_Once);
		ImGui::PushFont(DefaultFont);
		ImGui::Begin("Bloodhand v2.2 // Junkyyy#6185", NULL, 32994);
		ImVec2 P1, P2;
		...
		pDrawList->AddRectFilledMultiColor(P1, P2,
			...
			ImColor(0.446f, 0.025f, 0.025f, 1.000f),
			ImColor(0.264f, 0.023f, 0.023f, 1.000f),
			ImColor(0.000f, 0.000f, 0.000f, 1.000f));
		...
		else if (CFG.tab_index == 1)
		{
			...
			if (CFG.b_Aimbot)
			{
				...
				if (CFG.b_AimbotFOV)
				{
					ImGui::SliderFloat("FOV Size", &CFG.AimbotFOV, 1.0f, 300.f);
		...
		else if (CFG.tab_index == 2)
		{
			...
			//ImGui::Checkbox("Radar", &CFG.b_radar);
			//ImGui::Checkbox("TeamCheck", &CFG.b_TeamCheck);
			ImGui::Checkbox("Fast Reload (Use at own Risk)", &CFG.b_Fastreload);
			ImGui::Checkbox("Rapid Fire (Use at own Risk)", &CFG.b_Rapidfire);
			//ImGui::Checkbox("Fast Revive (Use at own Risk)", &CFG.b_fastres);
			if (ImGui::Button("Kill EAC"))
			{
				&CFG.b_eac1;
				MessageBox(NULL, _T("EAC DEAD \n Have a Great Day!"), _T("."), MB_OK | MB_SYSTEMMODAL);
			}
			//ImGui::Checkbox("Nothing", &CFG.b_test1);
			//ImGui::Checkbox("Nothing", &CFG.b_type1);
			//ImGui::Checkbox("Fast Reload", &CFG.b_Fastreload);
			//ImGui::Checkbox("Rapid Fire", &CFG.b_Rapidfire);
			//ImGui::Checkbox("No Spread (Use at own Risk)", &CFG.b_NoSpread);
			ImGui::BulletText("<3 Junkyyy Edits on YT <3");
			ImGui::BulletText("..........................................................");
		}
		ImGui::PopFont();
		ImGui::End();
...
int main()
{
	...
	printf("\nby Junkyyy#6185\n\nCredits to: Unknowncheats.me Jenrix, Ricecum1, shogust12, zOpen, vegacold1995 \n");
